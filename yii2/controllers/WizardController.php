...
class WizardController extends Controller {
    ...
    public function actionServicelist() {
		...
		if(($request->get('post_answers_ids'))!=NULL)
		{
		...
		if(!empty($request->get('post_answers_ids'))) {
			...
			$query->where(['yii_services.status' => 'A']);
			...
			$post_serviceindustry = $request->get('post_prestablishment');
 			//$query->where(['yii_answer_services_mapping.answer_id' => $post_answers_ids]);
			$query = $query->where(['yii_services.industry_status' => 'Pre Establishment','yii_services.status' => 'A']);
			$command = $query->createCommand();
			$servicelist = $command->queryAll();
			...
			$query->where(['yii_services.industry_status' => 'Pre Operational','yii_services.status' => 'A']);
			//$query->groupBy(['yii_answer_services_mapping.answer_id','yii_answer_services_mapping.service_id']);
			$command = $query->createCommand();
			$servicelist = $command->queryAll();
    ...
    public function actionPrestablish() {
        ...
        $questionnaire = $query->where(['abbr' => ['PRE_EST_WIZARD'] , 'status' => 'Y'])->limit(2)->all(); 
		...
		if(count($_POST) > 1) {
				...
				//	echo $command->getRawSql(); exit();
					...
					return $this->redirect(['caf/cafservices', 'projectId'=>$project_id,'type'=>'prestablishment']);
					// return $this->refresh();
				}
				//---------------------------------------------- 
		}
    ...
    public function actionPreOperational() {
        ...
        $query = Questionnaire::find();  
        $questionnaire = $query->where(['abbr' => ['PRE_OPE_WIZARD'] , 'status' => 'Y'])->limit(2)->all(); 
        ...
        $query_dep = Questions::find()->select('id, grp_id, text, render_type, parent_id , abbr_remark'); 
        $questions_depq = $query_dep->where(['status'=>'A' , 'abbr_remark'=>'sector'])->orderBy('disp_order')->all();

        //print_r($questions_depq); exit();

        $query_depq = Answers::find()->select('id, text, question_id, parent_ans_id, dependent_questions,pollution_remark');
        $answers_depq = $query_depq->where(['id'=>$investor_project[0]['sector'], 'question_id'=>$questions_depq[0]['id'] ,'status'=>'A'])->orderBy('disp_order')->all();

        $query_depqa = Answers::find()->select('dependent_questions');
        $answers_depqa = $query_depqa->where(['status'=>'A'])->groupBy('dependent_questions')->all();

        
        //echo "<pre>";
        //print_r($questions_depq);
        //echo "</pre>";
		...
		if(count($_POST) > 1) {
				...
				//	echo $command->getRawSql(); exit();
					...
					return $this->redirect(['caf/cafservices', 'projectId'=>$project_id,'type'=>'preoperational']);
					// return $this->refresh();
				}
				//---------------------------------------------- 
		}
        ...
        return $this->render('prestablish', [
            ...
            'questions' => $questions,
            'answers' => $answers,
            'answers_depq' => $answers_depq,
            'answers_depqa' => $answers_depqa,
            'questions_depq'=> $questions_depq,
			'investor_project' => $investor_project,
        ]);
