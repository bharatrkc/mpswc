...
$no_upload = 0;
 ...
 foreach($investorProjectDetail as $investorproject) {
	...
	if($investorproject->completed) {
 		echo  '<li><a href="'.Url::to(['investor/service', 'project_id'=>$investorproject->project_id, 'serviceid'=>$investorproject->service_id, 'action'=>'view', 'type' => $industry_status]).'" title=""><em>Step '.$i.': Services - '.$name.'</em><span> </span></a></li>';
	}
	else {
		if($flag == 0) {
			
			$active_class = "";
			if($service_id == $investorproject->service_id) {
				$active_class = ' class="current" ';
			}
			echo  '<li '.$active_class.'><a href="'.Url::to(['investor/service', 'project_id'=>$investorproject->project_id, 'serviceid'=>$investorproject->service_id,'type' => $industry_status]).'" title=""><em>Step '.$i.': Services - '.$name.'</em><span> </span></a></li>';
	...
	if($no_upload == 1) {
		echo  '<li><a href="#" title=""><em>Step '.$i.': Upload Enclosures</em><span></span></a></li>';
	}
	else {
		 echo  '<li><a href="'.Url::to(['caf/upload', 'project_id'=>$investorproject->project_id, 'type' => $industry_status]).'" title=""><em>Step '.$i.': Upload Enclosures</em><span></span></a></li>';
	}
}
?>
...
<div class="portlet box green">
	...
	<ul id="progressbar" style="display:none;"><?= $services_progress; ?></ul>
...
/*echo '<link rel="stylesheet" type="text/css" href="/backoffice/services/css/style.css">';
