...
<div class="wrapper wrapper-content animated fadeInRight"> 
				   ...
				   <table class="table table-bordered table-striped table-hover">
					...
					if(isset($investorproject_detail)) {
						if(count($investorproject_detail)) {
							$i = 1;
							$x = 1;
							foreach ($investorproject_detail as $key=>$service): 
								
							if($investorproject_detail[$key]['industry_status'] == 'Pre Establishment'  && $_GET['type']!='preoperational') {

								$flag = 1;
								if($flag == 1 && $i ==1) { $upperser = 1; echo $headerx; }
									
									$service_id = $investorproject_detail[$key]['sid'];	
									$disabled = '';
									$checked = '';
									$img = '';
									if($investorproject_detail[$key]['status']==2) {
										$disabled = 'disabled';
										$checked = 'checked'; 
									}
									if($investorproject_detail[$key]['completed']!=NULL) { 
										$img = '<img src="images/apply.png" style="width:20px;position: absolute;"/>';
									}
									?>
								   <tr>
									  <td class=" "><?php print $i; ?></td>
									  <td class="" style="width: 60%;"><?php if(isset($investorproject_detail[$key]['sname'])) { print $investorproject_detail[$key]['sname']; } ?></td>
	...
	<?php 
		...
		foreach ($investorproject_detail as $key1=>$service): 
				
			if($investorproject_detail[$key1]['industry_status'] == 'Pre Operational'  && $_GET['type']!='prestablishment') {
				
				$flag = 1;
				if($flag == 1 && $i ==1) { echo $header; }
				
						//echo '<pre>'; print_r($service); exit;
							$service_id = $investorproject_detail[$key1]['sid'];	

				$disabled = '';
				$checked = '';
				$img = '';
				if($investorproject_detail[$key1]['status']==2) {
					$disabled = 'disabled';
					$checked = 'checked'; 
				}
				if($investorproject_detail[$key1]['completed']!=NULL) {
					$img = '<img src="images/apply.png" style="width:20px;position: absolute;"/>';
				}
				?>
			     <tr>
				  <td class=" "><?php print $i; ?></td>
				  <td class=" " style="width: 60%;"><?php if(isset($investorproject_detail[$key1]['sname'])) { print  wordwrap($investorproject_detail[$key1]['sname'],100," \n"); } ?></td>
				  <td class=" " style="text-align:center;">
							...
							if($flag == 1 && $i ==1) { $upperserx = 1; echo $xheader; }
								   ...
								   <tr>
										...
										<button class="servdetail btn btn-sm btn-info" data-toggle="modal" data-target="#data__55" data-valids="<?php echo $service_all1['id']; ?>">View Details</button>
                                            ...
                                            <!--<a class="clklink" data-toggle="modal" data-target="#data__55">Click for details</a>-->
